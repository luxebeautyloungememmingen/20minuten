(function () {
  ...
  function saveState() {
    ...
    } catch(e) { /* sessionStorage voll oder disabled */ }
  }
  function loadState() {
    try {
      ...
      // Verfalldatum 4h -- danach Frischstart
      if (!s.ts || (Date.now() - s.ts) > 4*3600*1000) {
      ...
      isOpen        = !!s.isOpen;
      greeted       = !!s.greeted;
      bookingDone   = !!s.bookingDone;
      collected     = Object.assign(collected, s.collected || {});
      rejectedSlots = Array.isArray(s.rejectedSlots) ? s.rejectedSlots : [];
      return true;
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      // Quick replies - horizontal scrollbar mit Padding-Trick (letzter Button bekommt rechts Luft)
      '#fbw-qr-wrap{flex-shrink:0;border-top:1px solid #eef0ec;background:#fafbfa}',
      ...
      '#fbw-qr::after{content:"";flex-shrink:0;width:6px}',
  ...
  function init() {
    ...
    bindEvents();
    if (restored) {
      // State wiederhergestellt -- DOM mit alten Messages befuellen
      renderMsgs();
      if (isOpen) {
        document.getElementById('fbw-btn').classList.add('open');
        document.getElementById('fbw-win').classList.add('vis');
        var p = document.getElementById('fbw-pulse'); if(p) p.style.display='none';
        // Slots im Hintergrund neu laden (fuer Buchungen)
        loadSlots();
      }
    }
    renderQR();
